...
export default function LoginPage() {
  ...
  async function onSubmit(values: SignInInput) {
    ...
    if (error) { setServerError(mapAuthError(error.message)); return; }
    if (data.user) {
      const { data: profile } = await supabase.from("users").select("onboarding_completed").eq("id", data.user.id).single();
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        >
          <div className="space-y-1">
            <h1 className="font-display text-2xl font-[300]" style={{ color: "#F5F0E8" }}>Welcome back</h1>
            <p className="font-sans text-sm" style={{ color: "rgba(245,240,232,0.40)" }}>Your space is waiting.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
            ...
            >
              {isSubmitting ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Signing in…</span> : "Sign in"}
            </button>
          </form>
          ...
          <p className="text-center font-sans text-sm" style={{ color: "rgba(245,240,232,0.40)" }}>
            New to Serene?{" "}
            <Link href="/signup" className="text-sage-300 hover:text-sage-100 transition-colors">
              Create an account
            </Link>
          </p>
        </div>
